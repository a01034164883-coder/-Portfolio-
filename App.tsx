...
const SKILL_ICONS: Record<string,string> = {
  "Figma":"🎨","Photoshop":"🖼️","Illustrator":"✏️",
  "Premiere Pro":"🎬","After Effects":"✨","Meta Ads":"📊",
  "Instagram 운영":"📱","콘텐츠 기획":"📝",
};
        ...
        if (secId === "skills") return (
          <section key="skills" id="skills" ref={skillRef} className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {data.skills.map(sk => (
                  <div key={sk.id} className="skill-row" style={{ background: primary+"08", borderColor: primary+"15" }}>
                    <span className="text-2xl shrink-0">{SKILL_ICONS[sk.name]||"🔧"}</span>
            ...
            {data.tools && data.tools.length > 0 && (
              <div className="mt-5 card p-6 reveal" style={{ background: D.cardBgColor }}>
                <span className="text-[10px] font-black font-mono uppercase tracking-wider block border-b pb-2 mb-5" style={{ color: primary, borderColor: primary+"20" }}>핵심 실무 협업 도구</span>
                <div className="flex flex-wrap gap-4">
                  {data.tools.map(t => (
                    <div key={t.id} className="flex flex-col items-center gap-2 p-3 rounded-xl border cursor-default w-20 hover:scale-105 transition-transform"
                      style={{ borderColor: primary+"20", background: primary+"08" }}>
                      <div className="w-10 h-10 rounded-lg bg-white flex items-center justify-center overflow-hidden">
                        {t.iconUrl ? <img src={t.iconUrl} alt={t.name} className="w-7 h-7 object-contain" /> : <Settings className="w-5 h-5" style={{ color: primary }} />}
                      </div>
                      <span className="text-[9px] font-bold text-center" style={{ color: D.textColor }}>{t.name}</span>
                    </div>
                  ))}
                </div>
              </div>
